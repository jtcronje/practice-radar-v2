...
const PracticeOverview = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Load billing data
        const billingResponse = await window.fs.readFile('billing.csv', { encoding: 'utf8' });
        const billingParsed = Papa.parse(billingResponse, {
          header: true,
          dynamicTyping: true,
          skipEmptyLines: true
        });
        ...
        const proceduresResponse = await window.fs.readFile('procedures.csv', { encoding: 'utf8' });
        const proceduresParsed = Papa.parse(proceduresResponse, {
          header: true,
          dynamicTyping: true,
          skipEmptyLines: true
        });
        ...
        // Load doctors data
        const doctorsResponse = await window.fs.readFile('doctors.csv', { encoding: 'utf8' });
        const doctorsParsed = Papa.parse(doctorsResponse, {
          header: true,
          dynamicTyping: true,
          skipEmptyLines: true
        });
        ...
        // Load hospitals data
        const hospitalsResponse = await window.fs.readFile('hospitals.csv', { encoding: 'utf8' });
        const hospitalsParsed = Papa.parse(hospitalsResponse, {
          header: true,
          dynamicTyping: true,
          skipEmptyLines: true
        });
        
        setBillingData(billingParsed.data);
        setProceduresData(proceduresParsed.data);
        setDoctorsData(doctorsParsed.data);
        setHospitalsData(hospitalsParsed.data);
        
        // Extract unique procedures for the filter
        const procedures = _.uniqBy(proceduresParsed.data, 'Procedure Code').map(proc => ({
  ...
  const applyFilters = (data, dateField) => {
    ...
    // Apply date filter
    let filteredData = filterByDateRange(data, dateField);
    
    // Apply doctor filter if selected
    if (selectedDoctor) {
      filteredData = filteredData.filter(item => item['Provider ID'] === selectedDoctor);
    }
    
    // Apply location filter if selected
    if (selectedLocation) {
      filteredData = filteredData.filter(item => item['Location ID'] === selectedLocation);
    }
    
    // Apply procedure filter if selected
    if (selectedProcedure) {
      filteredData = filteredData.filter(item => item['Procedure Code'] === selectedProcedure);
    }
  ...
  const getFilteredBillings = () => {
    ...
    if (selectedDoctor || selectedLocation || selectedProcedure) {
      ...
      const procedureIds = filteredProcedures.map(p => p['Procedure Record ID']);
      
      // Filter billings that match these procedure IDs
      const filteredBillings = filterByDateRange(billingData, 'Date Billed / Claim Submit Date')
        .filter(billing => procedureIds.includes(billing['Procedure Record ID']));
      
      return filteredBillings;
    }
    
    // If no doctor/location/procedure filters, just apply date filter
    return filterByDateRange(billingData, 'Date Billed / Claim Submit Date');
  };
                ...
                >
                  ...
                  {uniqueProcedures.map(proc => (
                    <option key={proc.code} value={proc.code}>
                      {proc.code} - {proc.description.substring(0, 20)}
                      {proc.description.length > 20 ? '...' : ''}
                    </option>
                  ))}
                </select>
              </div>
